         ...
         std::string &v)
{
  ...
  for (unsigned i = 0; i < nodes.size(); i++) {
    ...
    if (cl) {
        if (cl->call(paxos_protocol::preparereq, me, arg, res, rpcc::to(1000))
                    == paxos_protocol::OK) {
            if (res.oldinstance) {
              ...
              tprintf("proposer::run: prepare reject\n");  
            }
        } else {
            tprintf("proposer::run: RPC return not OK\n");
        }
    } else {
        tprintf("proposer::run: safebind failed\n");
    }
  }
  return true;
}
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  for (unsigned i = 0; i < nodes.size(); i++) {
    ...
    if (cl && cl->call(paxos_protocol::acceptreq, me, arg, r, rpcc::to(1000)) 
            == paxos_protocol::OK && r);
        accepts.push_back(nodes[i]);
  }
}
	      ...
	      std::string v)
{
  ...
  for (unsigned i = 0; i < accepts.size(); i++) {
    ...
    if (cl && cl->call(paxos_protocol::decidereq, me, arg, r, rpcc::to(1000))
            != paxos_protocol::OK)
        tprintf("proposer::decide: RCP return not OK\n");
  }
}
...
acceptor::acceptreq(std::string src, paxos_protocol::acceptarg a, bool &r)
{
  ...
  ScopedLock ml(&pxs_mutex);
  tprintf("acceptor::acceptreq: for instance %d (my instance %d) v=%s\n",
