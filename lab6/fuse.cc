...
class log_file {
public:
	char type, *name, *content;
	int len, parent;
};

FILE *log_fopen() {
	FILE *logfile;
	char filename[200], mode[5];
	sprintf(filename, "./log/ver%d.log", ver);
	if (fclear==1) {
		fclear = 0;
		sprintf(mode, "w+");
	} else if (fclear==0) {
	...
	if (!(logfile = fopen(filename, mode))) {
		printf("[fuse.cc] log file open error\n");
		exit(0);
	}
	return logfile;
}

void log_read(fuse_ino_t parent, const char *name) {
	bool found;
	std::string data;
	yfs_client::inum ino;

	yfs->lookup(parent, name, found, ino);
	yfs->read(ino, 30000, 0, data);
	FILE *logfile = log_fopen();
	fputc('w', logfile);
	putw((int)data.size(), logfile);
	if (!strcmp(name, "f1")) {
		printf("[data.size] [%d]\n", data.size());
	}
...
// seems like this is not in the test
void log_write(fuse_ino_t ino, size_t size, off_t off, const char *buf) {
	if (nolog) return;
	FILE *logfile = log_fopen();
	fputc('w', logfile);
	putw(size, logfile);
	fwrite(buf, size, 1, logfile);
	//fprintf(logfile, "w %d,%.*s\n", (int)size, (int)size, buf);
	fclose(logfile);
}

void log_create(fuse_ino_t parent, const char *name, mode_t mode) {
	if (nolog) return;
	FILE *logfile = log_fopen();
	fputc('c', logfile);
	putw((int)parent, logfile);
	putw((int)strlen(name), logfile);
	fwrite(name, strlen(name), 1, logfile);
	fclose(logfile);
}
...
// seems like this is not in test
void log_mkdir(fuse_ino_t parent, const char *name, mode_t mode) {
	return;
	if (nolog) return;
	FILE *logfile = log_fopen();
...
void log_pre() {
	...
	nolog = 1;
	// read and undo
	char type, tmp_type, name[200], content[21000];
	size_t tmp;
	int parent, len=0, tmp_len=0;
	mode_t mode = S_IFREG | (0777);
	yfs_client::inum inum;
	std::vector<log_file> vlf;

	// scan into vector
	while (!feof(logfile)) {
		tmp_type = fgetc(logfile);
		if (tmp_type == 'w') {
			tmp_len = getw(logfile);
			fgets(content+len, tmp_len+1, logfile);
			len += tmp_len;
			printf("\t read type[w] len[%d] content[%.15s]\n", len, content);
			continue;
		} else if (len) {
			log_file lf;
			lf.type = type;
			...
			strcpy(lf.name, name);
			lf.content = (char*)malloc(sizeof(char)*len+1);
			memcpy(lf.content, content, len+1);
			lf.parent = parent;
			lf.len = len;
			vlf.push_back(lf);
			printf("[vlf] type[%c], name[%.10s], len[%d], content[%.10s]\n", type, name, len, content);
			len = 0;
		}
		...
		parent = getw(logfile);
		len = getw(logfile);
		fgets(name, len+1, logfile);
		len = 0;
		printf("\t read type[%c] parent[%d] name[%.10s]\n", type, parent, name);
	}

	// process vector
	for (int i=vlf.size()-1; i>-1; i--) {
		if (vlf[i].type == 'c') {
			yfs->unlink(vlf[i].parent, vlf[i].name);
		} else if (vlf[i].type == 'u') {
			yfs->create(vlf[i].parent, vlf[i].name, mode, inum);
			yfs->write(inum, vlf[i].len, 0, vlf[i].content, tmp);
		}
	}
...
void log_next() {
	...
	while (!feof(logfile)) {
		...
		if (tmp_type == 'w') {
			...
			fgets(content+len, tmp_len+1, logfile);
			len += tmp_len;
			printf("\t read type[w] tmp_len[%d] content[%.15s]\n", tmp_len, content);
			continue;
		}
		...
		if (len) {
			if (type == 'u') {
				printf("\t unlink\n");
				yfs->unlink(parent, name);
			} else if (type == 'c') {
				printf("\t create file\n");
				yfs->create(parent, name, mode, inum);
				yfs->write(inum, len, 0, content, tmp);
			}
			len = 0;
		}
		...
		len = getw(logfile);
		fgets(name, len+1, logfile);
		printf("\t read type[%c] len[%d] name[%.15s]\n", type, len, name);
		len = 0;
	}
...
fuseserver_symlink(fuse_req_t req, const char *path, fuse_ino_t parent, const char *name)
{
    ...
    if (( ret = yfs->symlink(parent, name, path, ino_out)) == yfs_client::OK) {
		printf("Symlink complete.\n");
		e.ino = ino_out;
...
main(int argc, char *argv[])
{
	...
	// implement version control using signal
